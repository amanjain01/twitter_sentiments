...
if __name__ == "__main__":
    ...
    #check()
    all_users = open("mb_users.txt","r",encoding = 'utf-8').readlines() #this file you cange for different hashtag
    arg.since = "2014-06-09"
    arg.year = "2014-10-09"
    arg.hashtags = True
    arg.count = True
    arg.stats = True
    ...
    for each in all_users:
        if i >= 7984: #This number you can change for altering range of users.
            each = each.strip()
            if each != "" and each[0] != '#':
                arg.u = each
                arg.o = "MB_USERS1/" + each + ".txt"
                loop = asyncio.get_event_loop()
                loop.run_until_complete(main())
                print(str(i) + "User Done\n")
        i += 1
        if i == 25000:
            break
